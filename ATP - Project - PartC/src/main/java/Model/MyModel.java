...
public class MyModel extends Observable implements IModel {
                    ...
                    try {
                        ...
                        for(int row = 0; row < Height; row++) {
                            for (int column = 0; column < Width; column++)
                                mazeArray[row][column] = maze.getMaze(row, column);
                        }
    ...
    private void SolveMyMaze(){
        try {
            Client client = new Client(InetAddress.getLocalHost(), 5401, new IClientStrategy() {
                ...
                public void clientStrategy(InputStream inFromServer, OutputStream outToServer) {
                    try {
                        startPosition.setPosition(characterPositionRow, characterPositionColumn);
                        ...
                        for (int i = 0; i < mazeSolutionSteps.size(); i++){
                            ...
                            rowsSolution.add(MS.getPositionRow());
                            columnsSolution.add(MS.getPositionColumn()) ;
                        }
                    } catch (Exception e) {
            ...
            if(!hint) {
                ...
                String solvingMethod = properties.getProperty("SolvingMethod");
        ...
        switch (direction) {
            ...
            case 9:
                if (checkMove(characterPositionRow - 1, characterPositionColumn + 1)){
                    if(maze.getMaze(characterPositionRow - 1, characterPositionColumn) == 0 || maze.getMaze(characterPositionRow, characterPositionColumn + 1) == 0 ) {
                        characterPositionRow--;
                        characterPositionColumn++;
                        break;
                    }
    ...
    @Override
    public void saveProperties(String typeMaze, String solvingMethod, String numOfThreads) throws IOException, ClassNotFoundException{
        ...
        properties.setProperty("TypeMaze", typeMaze);
        properties.setProperty("SolvingMethod", solvingMethod);
        properties.setProperty("NumberOfThreads", numOfThreads);
